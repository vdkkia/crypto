...
const processGoogleTrendsForAllKeywords = async (secondsToComplete = process.env.ACTIVE_SECONDS_PER_MIN) => {
  try {
    const startTime = Date.now();
    logger.info(`processing google trends for ${cleanKeywords.length} keywords in ${batches.length} batches`);
    const reqsPerSec = batches.length / secondsToComplete;
    logger.info(`sending ${reqsPerSec} requests per seconds`);
    const msBetweenReqs = Math.ceil((secondsToComplete * 1000) / batches.length);
    ...
    let resultToReport = results.map((res) => (typeof res === "object" ? 1 : res));
    const newBatches = prepareBatches(
      results.filter((res) => isNaN(res)).reduce((agg, keywordList) => [...agg, ...keywordList]),
      4
...
async function getTrendDataForBatch({
  ...
  try {
    logger.info(`getting data for batch ${batchNumber}/${totalBatches}: ${keywords.join(" | ")}`);
    ...
    logger.info(`received timeline data for batch ${batchNumber}/${totalBatches}`);
    ...
    if (axios.isCancel(err)) {
      ...
      logger.error(`job ${batchNumber} failed with status ${err?.response?.status} and code ${err.code}.`);
      logger.error(err.message);
      return -1;
    }
  }
