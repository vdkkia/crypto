...
const interestOverTime = async (keyword, period, agent) => {
  return new Promise((resolve, reject) => {
    googleTrends
      ...
      .then((res) => {
        resolve(JSON.parse(res).default.timelineData);
      })
      .catch((err) => {
        if (err.requestBody && err.requestBody.includes("Error 429 (Too Many Requests)!!")) {
          logger.error(agent.proxy.host + " is banned...");
